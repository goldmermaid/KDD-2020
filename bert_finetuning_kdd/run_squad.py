...
def main():
    ...
    parser.add_argument('--vocab_file',
                        type=str, default="vocab/vocab", required=True,
    ...
    parser.add_argument('--save_to_s3',
                       default=None,
                       help='s3 bucket to save models to.')
    parser.add_argument('--save_to_sagemaker_dir',
                       default=None,
                       help='Sagemaker directory to save models to.')
    ...
    if args.do_train: # first read in squad examples 
        try:
            train_examples = read_squad_examples(
                input_file=args.train_file, is_training=True, version_2_with_negative=args.version_2_with_negative)
        except:
            print('screwing up filepath somehow')
            train_examples = read_squad_examples(
                input_file=args.train_file[1:-1], is_training=True, version_2_with_negative=args.version_2_with_negative)         
    ...
    try:
        ...
        config = BertConfig.from_json_file(args.config_file[1:-1])
    ...
    if args.local_rank != -1:
        ...
        model = DDP(model) # apparently can just wrap the model in DDP just like torch 
    elif n_gpu > 1:
